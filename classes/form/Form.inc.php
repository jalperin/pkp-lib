...
class Form {
	...
	function smartyFormLanguageChooser($params, &$smarty) {
		...
		$returner .= '</select><input type="submit" class="button" value="'. PKPLocale::translate('form.submit'). '" onclick="changeFormAction(\'' . htmlentities($params['form'], ENT_COMPAT, LOCALE_ENCODING) . '\', \'' . htmlentities($params['url'], ENT_QUOTES, LOCALE_ENCODING) . '\'); return false" /></div>';
		return $returner;
	}
